...
Storage.prototype._uploadFile = function(command, file, options) {
    ...
    return this._getConnection().then(function(socket) {
        ...
        console.log('offset:' + (1 + protocol.FDFS_PROTO_PKG_LEN_SIZE + extBL));
 ...
 * 
 */
Storage.prototype._uploadSlaveFile = function(command, file, options) {
    var _self = this;
    return this._getConnection().then(function(socket) {
        logger.debug('start upload slave file to storage server [%s]', _self._name);
        var gf = helpers.id2gf(options.fileId);
        var group = gf.group;
        var filename = gf.filename;
        var fileNameBL = Buffer.byteLength(filename, _self.config.charset);

        // 封装header并发送
        var bodyLength = 2 * protocol.FDFS_PROTO_PKG_LEN_SIZE + protocol.FDFS_FILE_EXT_NAME_MAX_LEN + protocol.FDFS_FILE_PREFIX_MAX_LEN +
            fileNameBL + options.size;
        ...
        // 封装并发送body
        // 除file content外的内容
        var preBL = Buffer.byteLength(options.prefixName, _self.config.charset);
        var extBL = Buffer.byteLength(options.ext, _self.config.charset);

        var buffer = new Buffer(bodyLength - options.size);
        var offset = 0;
        buffer.fill(0);
        //写入文件名长度
        helpers.number2Buffer(fileNameBL, protocol.FDFS_PROTO_PKG_LEN_SIZE).copy(buffer, offset);
        offset += protocol.FDFS_PROTO_PKG_LEN_SIZE;
        //写入文件长度
        helpers.number2Buffer(options.size, protocol.FDFS_PROTO_PKG_LEN_SIZE).copy(buffer, offset);
        offset += protocol.FDFS_PROTO_PKG_LEN_SIZE;

        //写入文件名前缀
        buffer.write(options.prefixName, offset, preBL, _self.config.charset);
        offset += protocol.FDFS_FILE_PREFIX_MAX_LEN;
        console.log('offset:' + offset);

        //写入文件名后缀
        buffer.write(options.ext, offset, extBL, _self.config.charset);
        offset += protocol.FDFS_FILE_EXT_NAME_MAX_LEN;

        //写入文件名
        buffer.write(filename, offset, fileNameBL, _self.config.charset);
        ...
        return new Promise(function(resolve, reject) {
            protocol.recvPacket(
                ...
                function(err, body) {
                    ...
                    var fileId = _parseFileId4Slave(body, _self.config.charset);
                    resolve(fileId);
                }
            );
        });
...
Storage.prototype.upload = function(file, options) {
    ...
    } else if (options.method === protocol.FDFS_METHOD_UPLOAD_SLAVE_FILE) {
        var command = protocol.STORAGE_PROTO_CMD_UPLOAD_SLAVE_FILE;
        return this._uploadSlaveFile(command, file, options);
...
function _parseFileId4Slave(body, charset) {
    if (body && body.fileExist) {
        return {
            exist: true,
        }
    }
