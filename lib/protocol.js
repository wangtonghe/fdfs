...
Object.defineProperties(protocol, {
    ...
    //文件已存在
    HEADER_STATUS_FILE_ALREADY_EXIST: helpers.buildConstProp(17),
...
_.extend(protocol, {
    ...
    FDFS_METHOD_UPLOAD_SLAVE_FILE: 'uploadSalve',
...
function _parseHeader(headerBuffer, expectedCommand, expectedBodyLength) {
    ...
    if (status !== protocol.HEADER_STATUS_SUCCESS && status !== protocol.HEADER_STATUS_FILE_ALREADY_EXIST) {
        var err = new Error('receive packet errno is: ' + status);
        err.code = err.errno = status;
        throw err;
    }
    if (status === protocol.HEADER_STATUS_FILE_ALREADY_EXIST) {
